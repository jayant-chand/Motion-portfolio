...
const projects = [
  {
    id: 0,
    title: "AI Agent Job Search",
    category: "Agentic AI · LangChain · MCP · LLM · Telegram Bot",
    description:
      "Autonomous career monitoring agent built with Hermes Agent + TinyFish MCP tools. Crawls 25+ company career pages daily, ranks postings via a weighted scoring model (stack overlap, seniority, location), and auto-generates tailored cover letters and resume bullet points on command. Integrated OpenRouter LLM backend (Llama 3.3 70B / DeepSeek) with a Telegram bot gateway for ranked role alerts — deployed 24/7 on Oracle Cloud Free Tier with persistent cross-session memory.",
    link: "https://github.com/jayant-chand",
  ...
  },
  {
    ...
    description:
      "Production-grade Dockerized SaaS billing platform with PostgreSQL, Redis caching, Celery async queues, and CI/CD via GitHub Actions.",
  ...
  },
  {
    ...
    description:
      "IEEE-published deep learning pipeline (ResNext CNN + LSTM) achieving ~90% detection accuracy. Deployed as Dockerized REST inference service, ~150+ req/day.",
  ...
  },
  {
    ...
    description:
      "AI-driven SaaS for personalized travel itineraries via natural language prompts. Integrates Gemini, Google Maps routing, Redis caching, and OAuth auth.",
          ...
          {projects
            ...
            .map((proj) => (
              <motion.a
                ...
                className={`group relative md:col-span-2 h-[480px] rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl overflow-hidden transition-all duration-500 ${proj.borderHover} hover:bg-white/10 flex flex-col justify-end p-10 cursor-pointer shadow-2xl`}
              >
                <Image
                  src={proj.image}
                  alt={proj.title}
                  fill
                  className="object-cover absolute inset-0 z-0 opacity-25 group-hover:opacity-45 group-hover:scale-105 transition-all duration-700"
                />
                {/* Violet ambient glow */}
                <div className="absolute inset-0 bg-gradient-to-br from-violet-900/30 via-transparent to-transparent z-0" />
                ...
                <div className="absolute top-8 left-8 z-10 flex items-center gap-2">
                  <span className="flex items-center gap-1.5 px-3 py-1 rounded-full border border-violet-400/50 bg-violet-500/20 text-violet-300 text-xs font-semibold tracking-widest uppercase">
                ...
                <div className="relative z-10 flex justify-between items-end w-full">
                  <div className="max-w-2xl">
                    ...
                    <h4 className="text-4xl font-semibold tracking-tight mb-4 transition-transform duration-500 group-hover:-translate-y-2">
                      {proj.title}
                    </h4>
                    <p className="text-gray-400 text-sm font-light leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity duration-500 translate-y-4 group-hover:translate-y-0 max-w-xl">
                      {proj.description}
                    </p>
                  </div>
